...
const urlTobase64 = async (url) => {
  try {
    ...
    // detect content type (optional, here assuming jpeg)
    const contentType = response.headers.get("content-type") || "image/jpeg";
    const dataUrl = `data:${contentType};base64,${base64}`;
    console.log("Converted URL to base64:", dataUrl);
...
export const videoInfo = async (req, res) => {
  ...
  try {
    ...
    const info = await ytdlp(url, {
      ...
      addHeader: ["referer:instagram.com", "user-agent:instagrambot"],
    });
    ...
    const imageBase64 = await urlTobase64(info.thumbnail || info.thumbnails?.[0]?.url || "");
    if (!imageBase64) {
      console.warn("Failed to convert thumbnail to base64");
    }
